...
export default function ProfileClient({ user, problems }: { user: User, problems: ProblemWithLogs[] }) {
    ...
    const usernameHandle = useMemo(() => {
        if (!user?.name) return "engineer";
        return user.name.toLowerCase().replace(/[^a-z0-9]/g, "");
    }, [user]);

    const revealClass = `transition-all duration-500 ease-out ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2"
        }`;
                                            ...
                                            <div key={wIdx} className="flex flex-col gap-[3px]">
                                                {week.map((day) => (
                                                    <div
                                                        ...
                                                        className={`w-[11px] h-[11px] sm:w-[12px] sm:h-[12px] rounded-[2.5px] transition-all cursor-pointer ${day.isEmpty || day.isFuture
                        ...
                        {recentRevisions.length === 0 ? (
                            ...
                            <div className="space-y-2.5 my-auto">
                                {recentRevisions.map((log) => (
                                    <div
                                        key={log.id}
                                        className={`flex items-center justify-between p-3.5 rounded-r-xl border-l-2 ${FEEDBACK_BORDER[log.userFeedback] || "border-l-zinc-700"
                                            } bg-zinc-900/30 hover:bg-zinc-900/70 transition-colors`}
                                    >
                                        <div className="flex items-center gap-3 min-w-0">
                                            <span
                                                className={`text-[10px] uppercase font-bold shrink-0 ${FEEDBACK_TEXT[log.userFeedback] || "text-zinc-500"
                                                    }`}
                                            >
                                                {log.userFeedback}
                                            </span>
